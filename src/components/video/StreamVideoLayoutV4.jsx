...
const StreamVideoLayoutV4 = ({ callId }) => {
  const { useCallCallingState, useParticipants, useIsCallRecordingInProgress } =
    useCallStateHooks();
  const call = useCall();
  ...
  // 🔴 NEW: mic-muted popup state
  const [, setIsMicMuted] = useState(false);
  const [showMicMutedWarning, setShowMicMutedWarning] = useState(false);
  const micWarningTimerRef = useRef(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // 🔴 NEW: clear mic warning timer when component unmounts
      if (micWarningTimerRef.current) {
        clearTimeout(micWarningTimerRef.current);
        micWarningTimerRef.current = null;
      }
      setShowMicMutedWarning(false);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [call]);
  ...
  const handleCancel = async () => {
    ...
    // 🔴 NEW: also clear popup when call ends
    if (micWarningTimerRef.current) {
      clearTimeout(micWarningTimerRef.current);
      micWarningTimerRef.current = null;
    }
  ...
  // 🔴 NEW: when mic button is clicked
  const handleMicToggleClick = () => {
    ...
    setIsMicMuted((prev) => {
      ...
      if (nextMuted) {
        // mic just muted → start 5-second timer
        micWarningTimerRef.current = setTimeout(() => {
          setShowMicMutedWarning(true);
        }, 5000); // 5 seconds
      } else {
        // mic unmuted → hide warning
        setShowMicMutedWarning(false);
      }
  ...
  return (
    <StreamTheme>
      ...
      >
        {/* 🔴 NEW: wrap mic button so we can detect clicks */}
        <div onClick={handleMicToggleClick}>
          <ToggleAudioPreviewButton />
        </div>
      ...
      {/* 🔴 NEW: visible warning when mic muted for 5 seconds */}
      {showMicMutedWarning && (
        <div
          style={{
            marginTop: "8px",
        ...
        >
          Your mic is muted. Please turn on the microphone.
        </div>
      )}
    </StreamTheme>
  );
