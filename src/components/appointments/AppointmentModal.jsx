...
const AppointmentModal = ({ selectedAppointment, setSelectedAppointment }) => {
    ...
    const isSeismified = selectedAppointment?.seismified === true;
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 w-[90%] max-w-md">
                ...
                <div className="space-y-2 text-gray-700">
                    ...
                    <p>
                        ...
                        <Link to={`/patients/${selectedAppointment.patient_id}`} target="_blank" className="text-blue-600 hover:underline hover:text-blue-800 transition-colors">
                    ...
                    <p>
                        ...
                        {isSeismified ? (
                            ...
                            <span className="text-gray-500 font-medium">Not Seismified</span>
                        )}
                    </p>
                    ...
                    <div className="flex space-x-4 mt-2">
                        <label className="flex items-center space-x-1">
                            <input type="radio" name="apptType" value="online" checked={isOnline} onChange={() => setIsOnline(true)} />
                            <span>Online</span>
                        </label>
                        <label className="flex items-center space-x-1">
                            <input type="radio" name="apptType" value="inperson" checked={!isOnline} onChange={() => setIsOnline(false)} />
                            <span>In-Person</span>
                        </label>
                    </div>
                    ...
                    {isOnline && (
                        <>
                            ...
                            <div className="flex w-full">
                                <input type="text" value={joinLink} readOnly className="flex-grow border border-gray-300 rounded-l-md px-4 py-2 focus:outline-none focus:ring-0" />
                ...
                <div className="mt-6 text-right space-x-1">
                    ...
                    <button
                        ...
                        disabled={!isSeismified}
                        className={`py-2 px-4 rounded font-medium ${isSeismified ? 'bg-zinc-600 hover:bg-zinc-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
                    >
                        Post Call Documentation
                    </button>
                    <button onClick={() => setSelectedAppointment(null)} className="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded">Close</button>
                </div>
            </div>
        </div>
    );
