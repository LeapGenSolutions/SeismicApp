...
import { FaUserMd, FaVideo, FaPlus, FaList } from "react-icons/fa";
...
const AppointmentStats = ({ date: propDate }) => {
  ...
  useEffect(() => {
    ...
    // Count by mode
    const inPersonAppointments = todayAppointments.filter(app => app.mode === 'in-person').length;
    const virtualAppointments = todayAppointments.filter(app => app.mode === 'virtual' || app.mode === 'online').length;
    const totalAppointments = inPersonAppointments + virtualAppointments;
    setStats({
      totalAppointments,
