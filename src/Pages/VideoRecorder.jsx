  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <PageNavigation 
          title="Video Call"
          subtitle="Start or join a video consultation"
          showBackButton={true}
        />
      </div>
      ...
      {/* Floating Notepad Components - Only show for doctors */}
      {role === "doctor" && (
        <>
          {/* <NotesTrigger
            onToggle={() => setIsNotepadOpen(!isNotepadOpen)}
            isOpen={isNotepadOpen}
            patientName={selectedAppointment?.full_name}
          /> */}
      ...
      {isNotepadOpen && (
        <div className="fixed bottom-0 left-0 right-0 bg-green-600 text-white text-center py-2 text-sm font-medium z-30">
          🟢 Notes Active - {selectedAppointment?.full_name ? `Patient: ${selectedAppointment.full_name}` : 'Quick Notes'}
        </div>
      )}
      */}
    </div >
