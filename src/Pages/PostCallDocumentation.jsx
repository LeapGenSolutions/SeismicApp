...
import CallFeedback from "../components/post-call/PostCallFeedback"; // renamed
...
const PostCallDocumentation = ({ onSave }) => {
  ...
  return (
    <>
      {/* Top bar with Back button and Call Feedback */}
      <div className="flex justify-between items-start mb-4 relative">
        ...
        {/* Call Feedback in top-right corner */}
      ...
      <Card className="mt-8">
          ...
          <div className="bg-white border border-gray-300 rounded-xl shadow p-6 mb-6 mx-6 md:mx-auto md:max-w-5xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-gray-800">
              ...
              <p className="flex items-center gap-2">
                ...
                <span className="font-medium text-gray-900">{ssn ?? '—'}</span>
              </p>
            </div>
        ...
        <CardContent>
          ...
          {docTab === "summary" && <Summary username={username} appointmentId={callId} />}
          {docTab === "transcript" && <Transcript username={username} appointmentId={callId} />}
          {docTab === "SOAP" && <Soap username={username} appointmentId={callId} />}
          {docTab === "recommendations" && <Reccomendations username={username} appointmentId={callId} />}
          {docTab === "billing" && <Billing username={username} appointmentId={callId} />}
          {docTab === "clusters" && <Clusters username={username} appointmentId={callId} />}
          {docTab === "doctor notes" && <DoctorNotes username={username} appointmentId={callId} />}
          {docTab === "emotional connect" && selectedAppointment && (
            <EmotionalConnect
              username={username}
              appointmentId={callId}
              patientId={selectedAppointment.patient_id ?? selectedAppointment.patient_Id}
            />
          )}
        </CardContent>
      </Card>
